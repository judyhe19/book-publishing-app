...
const BOOK_SALES_COLUMNS = [
  ...
  },
  {
    ...
    render: (sale) => {
      const total =
        sale.author_details?.reduce((sum, a) => sum + Number(a.royalty_amount), 0) || 0;
  ...
  },
  {
    label: "Status",
    sortKey: "paid_status",
    render: (sale) => <PaymentStatusBadge status={getPaymentStatus(sale)} />,
  },
  {
    label: "Actions",
    type: "actions",
    getActions: (sale) => [{ label: "Modify", to: `/sale/${sale.id}`, variant: "primary" }],
  },
];
