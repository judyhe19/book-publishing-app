#!/usr/bin/env python3
"""
Script to populate the database with EV3 sample data from CSV files.

Run:
  python3 src/scripts/load_ev3_data.py --env local
  python3 src/scripts/load_ev3_data.py --env prod
"""
...
BOOKS_CSV = os.path.join(PROJECT_ROOT, "data/ev3-sample-data/books.csv")
RECORDS_CSV = os.path.join(PROJECT_ROOT, "data/ev3-sample-data/records.csv")
COVERS_DIR = os.path.join(PROJECT_ROOT, "data/ev3-sample-data/img")
...
def import_books(session, base_url, books_csv_path):
            ...
            books_data.append(
                {
                    ...
                    "distributor_royalty_rate": float(row.get("royalty_percent_distribution", 0)) / 100,
                    "handsold_royalty_rate": float(row.get("royalty_percent_handsold", 0)) / 100,
    ...
    for book in books_data:
        ...
        payload = {
            ...
            "amazon_asin_ebook": book["asin"] if book["asin"] else None,
        ...
        elif resp.status_code == 400 and ("unique" in resp.text.lower() or "already exists" in resp.text.lower()):
            print(f"✓ Skipped: {book['title']} (already exists)")
            # Might want to upload cover if not exists? Skip for simplicity.
        else:
...
def import_sales(session, base_url, records_csv_path):
    ...
    with open(records_csv_path, "r", encoding="utf-8") as f:
        ...
        for row in reader:
            ...
            quantity = row.get("qty_sold", "").strip()
            quantity = int(quantity) if quantity else None
            
            kenp = row.get("kenp", "").strip()
            kenp = int(kenp) if kenp else None
            
            royalty_paid = (row.get("author_paid", "n") or "n").lower() == "y"
            ...
            elif source == "handsold":
                 sale_data["currency"] = "USD"
                 # handsold automatically calculates revenue
